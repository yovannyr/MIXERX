...
using IEngineService = MIXERX.Infrastructure.Services.IEngineService;
...
<<<<<<< TODO: Nicht zusammengeführte Änderung von Projekt „MIXERX.MAUI (net9.0-windows10.0.19041.0)“, Vor:
            var success = await MIXERX.Engine.Audio.Mp3Converter.ConvertWavToMp3Async(wavPath, _recordingPath);
=======
            var success = await Mp3Converter.ConvertWavToMp3Async(wavPath, _recordingPath);
>>>>>>> Nach

<<<<<<< TODO: Unmerged change from project 'MIXERX.MAUI (net9.0-windows10.0.19041.0)', Before:
            var success = await Infrastructure.Audio.Mp3Converter.ConvertWavToMp3Async(wavPath, _recordingPath);
=======
            var success = await Mp3Converter.ConvertWavToMp3Async(wavPath, _recordingPath);
>>>>>>> After

<<<<<<< TODO: Unmerged change from project 'MIXERX.MAUI (net9.0-windows10.0.19041.0)', Before:
            var success = await Infrastructure.Implementations.Audio.Mp3Converter.ConvertWavToMp3Async(wavPath, _recordingPath);
=======
            var success = await Mp3Converter.ConvertWavToMp3Async(wavPath, _recordingPath);
>>>>>>> After
            var success = await Infrastructure.Services.Audio.Mp3Converter.ConvertWavToMp3Async(wavPath, _recordingPath);
